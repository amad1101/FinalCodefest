<!-- Death in family for the two other branches -->

<template>
  <div class="constructionB">
    <Situation :context='job[1].choice[1].Situation' ></Situation>
    <br>
    <br>
    <br>
    <b-container >
      <b-row class="mb-3">
        <!-- <b-col md="4"> -->
          <Choice :key="index" v-for="(number, index) in job[1].choice[1].choice" :context='number.name'  :propB='number.cost' :propC='number.distance'  :propE='number.choiceName' :propF='number.link'></Choice>
          <!-- v-for="(number, index) in job[1].choice[0].choice[0].choice" :key="index" :context='number.name'  :propB='number.cost' :propC='number.distance'  :propE='number.choiceName' -->
          <!-- propA='' propD='' -->
